...
import { SkinQuizSection } from './SkinQuizSection';
...
const BeforeAfterSlider: React.FC<BeforeAfterSliderProps> = ({
  ...
  useEffect(() => {
    if (isInView && !hasUserInteracted) {
      ...
      const timer = setTimeout(() => {
        if (isCancelled || hasUserInteracted) return;
        animateSlider(50, 25, 1200, () => {
          if (isCancelled || hasUserInteracted) return;
          animateSlider(25, 75, 1600, () => {
            if (isCancelled || hasUserInteracted) return;
            animateSlider(75, 50, 1200, () => {});
          });
        });
      }, 1000);
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-5 left-5 bg-stone-900/80 backdrop-blur-md text-stone-100 text-[10px] sm:text-xs font-mono font-medium py-1.5 px-3.5 uppercase tracking-widest rounded-[4px] select-none pointer-events-none">
        BEFORE
      </div>
...
const KeyIngredientsSection: React.FC<KeyIngredientsSectionProps> = ({ product }) => {
  const leftIngredients = [
    {
      ...
      desc: "Deeply nourishes and moisturizes dry skin. Rich in vitamins A, D, and E, it helps improve skin elasticity and smoothness.",
  ...
  return (
    <div id="key-ingredients-section" className="max-w-7xl mx-auto px-4 sm:px-6 py-28 text-center relative overflow-hidden">
      ...
      <motion.div 
        ...
        className="grid grid-cols-[1fr_1.8fr_1fr] md:grid-cols-[1fr_2fr_1fr] lg:grid-cols-12 gap-1 sm:gap-4 lg:gap-4 items-center"
      >
        {/* LEFT COLUMN: 2 ingredients */}
        <div id="left-ingredients-col" className="col-span-1 lg:col-span-4 flex flex-col gap-6 sm:gap-16 order-1">
          {leftIngredients.map((ing, idx) => (
            ...
            >
              {/* Image Frame with a soft drop shadow, no circle boundary for freestanding look */}
              <div className="w-16 h-16 sm:w-28 sm:h-28 flex items-center justify-center relative mb-2 sm:mb-5 transition-transform duration-500 hover:scale-105 rounded-full overflow-hidden bg-white/50">
                <img decoding="async" loading="lazy" 
                  src={ing.image} 
                  alt={ing.title} 
              ...
              <h3 className="font-serif text-[11px] sm:text-[22px] text-stone-900 font-normal tracking-tight mb-1 sm:mb-2.5">
                {ing.title}
              </h3>
              <p className="hidden sm:block text-stone-500 font-sans text-[9px] sm:text-[14px] font-light leading-[1.2] sm:leading-relaxed tracking-wide">
                {ing.desc}
              </p>
            </motion.div>
          ))}
        ...
        <motion.div 
          ...
          variants={itemVariants}
          className="col-span-1 lg:col-span-4 flex justify-center items-center order-2 py-2 lg:py-6"
        >
          <div className="relative w-full max-w-[200px] sm:max-w-[380px] md:max-w-[500px] lg:max-w-full flex justify-center items-center">
            ...
            {isKiraCream ? (
              <motion.div 
                ...
                className="w-full aspect-square sm:w-[380px] sm:h-[380px] sm:w-[500px] sm:h-[500px] lg:w-[620px] lg:h-[620px] flex items-center justify-center cursor-pointer scale-[1.3] sm:scale-100"
                animate={{
                  y: [0, -12, 0],
                }}
                transition={{
              ...
              <motion.div 
                ...
                className="w-full aspect-square sm:w-[380px] sm:h-[380px] md:w-[500px] md:h-[500px] lg:w-[620px] lg:h-[620px] flex items-center justify-center cursor-pointer scale-[1.3] sm:scale-100"
                animate={{
                  y: [0, -12, 0],
                }}
                transition={{
              ...
              <motion.img 
                ...
                className="w-full h-auto aspect-square sm:w-auto sm:h-[380px] md:h-[500px] lg:h-[620px] object-contain cursor-pointer filter drop-shadow-[0_15px_30px_rgba(0,0,0,0.04)] scale-[1.3] sm:scale-100"
                animate={{
                  y: [0, -12, 0],
                }}
                transition={{
        ...
        <div id="right-ingredients-col" className="col-span-1 lg:col-span-4 flex flex-col gap-6 sm:gap-16 order-3">
...
export const ProductMain: React.FC<ProductMainProps> = ({
  ...
  return (
        ...
        <div className="lg:col-span-5 flex flex-col mt-2 lg:mt-0 select-none">
          ...
          <div className="flex items-center gap-3 sm:gap-4 mt-2 mb-6 select-none flex-wrap">
            {finalCompareAtPrice ? (
              <>
                ...
                {/* Save Percentage Badge & Scarcity Countdown */}
                {discountPercent > 0 && (
                  <div className="flex items-center gap-2 shrink-0 ml-1 flex-wrap">
                    <motion.div 
                      animate={{ rotate: [0, -4, 4, -4, 4, 0], scale: [1, 1.05, 1, 1.05, 1, 1] }}
                      transition={{ duration: 0.6, repeat: Infinity, repeatDelay: 7, ease: "easeInOut" }}
                      className="inline-flex items-center bg-[#AC1D22] text-white px-3 py-1.5 rounded-[6px] font-sans font-bold text-xs sm:text-[13px] tracking-wide shadow-xs shrink-0"
                    >
                      <svg className="w-4 h-4 mr-1.5 shrink-0" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path 
                          d="M2.5 8.5L8.5 2.5H13.5V7.5L7.5 13.5L2.5 8.5Z" 
                          fill="#FCE7E7" 
                          stroke="#AC1D22" 
                          strokeWidth="1.2" 
                          strokeLinejoin="round"
                        />
                        <circle cx="10.5" cy="5.5" r="1" fill="#AC1D22" />
                      </svg>
                      <span className="uppercase">SAVE {discountPercent}%</span>
                    </motion.div>
                    
                    {/* Time scarcity indicator */}
                    <div className="inline-flex items-center gap-1.5 bg-amber-50 text-amber-800 border border-amber-200 px-2.5 py-1 rounded-[6px] font-sans text-[11px] sm:text-xs font-medium tracking-wide shadow-2xs select-none">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse shrink-0" />
                      <span className="text-amber-700">Offer ends in</span>
                      <span className="font-mono font-bold tabular-nums text-amber-900">{formatTime(timeLeft)}</span>
                    </div>
                  </div>
                )}
              </>
            ...
            <ul className="space-y-2.5 pl-0.5">
              {getProductBullets(product.id).map((bullet, idx) => (
                <li key={idx} className="flex items-start gap-3.5 text-[13px] sm:text-sm text-stone-800">
                  <span className="font-bold mt-[1px] text-base leading-none select-none" style={{ color: themeColor }}>•</span>
          ...
          <div className="mt-5 mb-5 select-none space-y-1.5 pt-1.5">
            <div className="text-[11px] sm:text-xs font-semibold text-[#107048] tracking-wide flex items-center gap-1.5">
              Hurry, only {itemsLeft} items left in stock!
            </div>
            <div className="w-full bg-[#e6ebe7] h-1 rounded-full overflow-hidden">
              <motion.div 
                initial={{ width: 0 }}
                animate={{ width: `${(itemsLeft / 15) * 100}%` }}
                transition={{ duration: 1.5, ease: "easeOut", delay: 0.2 }}
                className="bg-[#107048] h-full rounded-full" 
              />
            </div>
          </div>
          ...
          <div className="mt-4 space-y-3">
            ...
            <LuxuryButton
              ...
              label="Add to cart"
              className="w-full h-[52px] text-center text-xs sm:text-sm font-semibold cursor-pointer rounded-none hover:opacity-90 transition duration-150 flex items-center justify-center gap-2 shadow-sm"
              style={{ backgroundColor: themeColor, color: '#1c1917' }}
            >
            ...
            <LuxuryButton
              type="button"
              label="Buy it now"
              className="w-full h-[52px] text-xs sm:text-sm font-semibold border bg-white transition duration-150 rounded-none cursor-pointer flex items-center justify-center"
          ...
          <div className="mt-6 space-y-2.5">
            ...
            {[
              { id: 1, label: 'Buy 1 item', price: convertAndFormatPrice('$' + baseUsdPrice, activeCurrency), orig: convertAndFormatPrice(finalCompareAtPrice || '$150.00', activeCurrency), pct: 'Standard Price' },
              { id: 2, label: 'Buy 2, save 20%', price: convertAndFormatPrice('$' + ((baseUsdPrice * 2) * 0.8), activeCurrency), orig: convertAndFormatPrice('$' + (baseUsdPrice * 2), activeCurrency), pct: 'Save 20%' },
              { id: 3, label: 'Buy 3, save 30%', price: convertAndFormatPrice('$' + ((baseUsdPrice * 3) * 0.7), activeCurrency), orig: convertAndFormatPrice('$' + (baseUsdPrice * 3), activeCurrency), pct: 'Best Choice' },
            ].map((tier, idx) => {
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      {tier.pct === 'Best Choice' ? (
                        <span 
                          className="inline-block text-[9px] font-bold px-2 py-0.5 rounded-none mt-0.5 uppercase tracking-wider"
                          style={{
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 bg-white py-3.5 sm:py-4.5 overflow-hidden border-t border-stone-200 shadow-sm">
        <div className="relative w-full flex overflow-hidden">
          <motion.div
            ...
            className="flex whitespace-nowrap gap-12 text-[10px] sm:text-[11px] font-mono tracking-[0.25em] uppercase font-bold text-stone-800 pr-12 shrink-0"
          >
            {[...Array(6)].map((_, i) => (
              <div key={i} className="flex items-center gap-12 shrink-0">
                <span>Brightening support with vitamins & clinical active extracts</span>
                <span className="text-[#00BCD4] font-sans">•</span>
    ...
    <div className="border-t border-stone-200/60 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-28 grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8 items-center">
        ...
        <div className="md:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-12 sm:gap-8 pl-0 md:pl-12">
          <AnimatedMetric
            value={84}
            label="saw better hydration in 1 week"
            delay={0.15}
            color={themeColor}
          />
          <AnimatedMetric
            value={90}
            label="less redness in 14 days"
            delay={0.3}
            color={themeColor}
          />
        </div>
    ...
    />
      ...
      <motion.div
        ...
        className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 bg-white/95 backdrop-blur-sm border border-stone-200 shadow-[0_8px_20px_rgba(0,0,0,0.12)] p-1.5 flex items-center gap-2 rounded-full w-[max-content]"
        style={{ pointerEvents: showStickyBar ? 'auto' : 'none' }}
      >
        <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 ml-0.5">
          <img decoding="async" loading="lazy" 
            src={(activeImageIdx === 0 && getCustomProductImage(product.id)) || product.images[activeImageIdx] || product.images[0]} 
            alt={product.title} 
            className="w-full h-full object-cover" 
        ...
        <button
          type="button"
          onClick={handleAddToCart}
          className="relative overflow-hidden h-8 px-5 text-center text-[11px] sm:text-xs font-semibold cursor-pointer rounded-full hover:opacity-90 transition duration-150 flex items-center justify-center gap-1.5 shadow-sm whitespace-nowrap"
          style={{ backgroundColor: themeColor, color: '#1c1917' }}
        >
          <ShoppingCart className="w-3.5 h-3.5 text-stone-900 shrink-0 hidden sm:block" />
          <span>Add to cart</span>
          <span className="opacity-70 ml-1 font-mono text-[10px] hidden sm:inline">• {convertAndFormatPrice('$' + baseUsdPrice, activeCurrency)}</span>
          {/* Subtle shine animation */}
          <motion.div
            className="absolute top-0 bottom-0 left-0 w-6 bg-white/40 skew-x-[-20deg]"
            animate={{ x: ['-200%', '400%'] }}
