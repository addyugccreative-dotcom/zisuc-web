...
export const LuxuryButton: React.FC<LuxuryButtonProps> = ({
  ...
  return (
    ...
    >
      ...
      {showShine && <div className="animate-button-shine" />}

      <span className="relative z-10 flex items-center justify-center gap-2 h-full py-0.5 pointer-events-none">
        {children && (
        ...
        {characters.length > 0 ? (
          <span className="flex items-center justify-center gap-x-[0.5px]">
            {characters.map((char, idx) => {
              ...
              const primaryY = isHovered ? '-100%' : '0%';
              const primaryOpacity = isHovered ? 0 : 1;

              const secondaryY = isHovered ? '0%' : '100%';
              const secondaryOpacity = isHovered ? 1 : 0;

              const delay = idx * 0.03; // Perfectly timed stagger matching the video exactly
