...
// Helper to get optimized source dynamically
const getResponsiveVideoUrl = (url: string) => {
  if (!url) return '';
  ...
  return isMobile ? `${base}_mobile.mp4` : `${base}_desktop.mp4`;
};

const UgcMarqueeVideo: React.FC<{ src: string; poster?: string; className?: string }> = ({ src, poster, className }) => {
  return (
    <OptimizedVideo
      src={src}
      poster={poster}
        ...
        {selectedVideo && (
          ...
          >
            ...
            >
              ...
              <video
                ref={modalVideoRef}
                src={getResponsiveVideoUrl(selectedVideo.videoUrl)}
                autoPlay
                loop
                playsInline
